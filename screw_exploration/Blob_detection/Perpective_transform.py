...
def detect_and_match_features_mser(webcam_img, kp_ref, des_ref, mser, orb):
    kp_frame = mser.detect(webcam_img)
    if des_ref is None or len(des_ref) < 2 or len(kp_frame) < 2:
        print("Not enough descriptors found.")
        return kp_ref, kp_frame, []
    
    # kp_frame = [cv2.KeyPoint(x[0], x[1], 1) for x in kp_frame]
    kp_frame_orb, des_frame_orb = orb.compute(webcam_img, kp_frame)
    
    webcam_img_kp = cv2.drawKeypoints(webcam_img, kp_frame, None, color=(0, 255, 0), flags=0)
    cv2.imshow("Keypoints", webcam_img_kp)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    return match_features(des_ref, des_frame_orb, kp_ref, kp_frame_orb, use_fginn=False)


def detect_and_match_features_orb(webcam_img, flann, kp_ref, des_ref, orb, ref_img_kp):
    ...
    web_img_kp = cv2.drawKeypoints(webcam_img, kp_frame, None, color=(0, 255, 0), flags=0)
    cv2.imshow("Keypoints", web_img_kp)
    cv2.imshow("ref image", ref_img_kp)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
