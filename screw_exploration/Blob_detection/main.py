...
from Perpective_transform import detect_and_match_features_mser, detect_and_match_features_sift, find_homography_and_transform, draw_matches, detect_and_match_features_surf, detect_and_match_features_orb
import time
from preprocess import preprocess_proposal
...
def test_with_image(reference_image, webcam_frame):
    ...
    # webcam_img = cv2.resize(webcam_img, (640, 480))
    ...
    # mser.setMinArea(100)
    # mser.setMaxArea(300)
    # kp_ref = mser.detect(preprocess_proposal(ref_img))
    # kp_ref = [cv2.KeyPoint(x[0], x[1], 1) for x in kp_ref]
    # kp_ref, des_ref = orb.compute(preprocess_proposal(ref_img), kp_ref)
    # beb = cv2.xfeatures2d.BEBLID_create(0.75)
    kp_ref, des_ref = orb.detectAndCompute(preprocess_proposal(ref_img), None)
    ref_img_kp = cv2.drawKeypoints(ref_img, kp_ref, None, color=(0, 255, 0), flags=0)
    # cv2.imshow("Keypoints", ref_img_kp)
    # _, des_ref = beb.compute(preprocess_proposal(ref_img), kp_ref)
    #kp_ref, kp_frame, good_matches = detect_and_match_features_mser(preprocess_proposal(webcam_img), kp_ref, des_ref, mser, orb)
    kp_ref, kp_frame, good_matches = detect_and_match_features_orb(preprocess_proposal(webcam_img), flann, kp_ref, des_ref, orb, ref_img_kp)
    # kp_ref, kp_frame, good_matches = detect_and_match_features_asift(ref_img, webcam_img)
    ...
    if matched:
        ...
        res_img = detect_using_locations(detect_img, new_screw_positions, model_yolo)
...
def main(reference_image_path, video_path=0, fps_limit=10, output_file="matches_count.txt"):
    ...
    with open(output_file, "w") as f:
        while cap.isOpened():
            ...
            if frame_count % frame_count_interval == 0:
                ...
                if matched:
                    cv2.imwrite("test_images/match_found.jpeg", frame)
                    frame_with_detections = detect_using_locations(frame, transformed_positions, model_yolo)
...
if __name__ == '__main__':
    # main("test_images/main_ref.jpg")
    test_with_image("Test_new/match2.jpg", "test_images/screw_test4.png")
    # test_with_image("test_images/calc_ref.jpeg", "test_images/calc_match.jpeg")
